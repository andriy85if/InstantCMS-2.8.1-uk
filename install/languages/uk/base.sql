...
CREATE TABLE `{#}controllers` (
  ...
  `files` varchar(4000) DEFAULT NULL COMMENT 'Список файлів контролера (для сторонніх компонентів)',
...
CREATE TABLE `{#}widgets` (
  ...
  `files` varchar(4000) DEFAULT NULL COMMENT 'Список файлів віджету (для сторонніх віджетів)',
  `addon_id` int(11) UNSIGNED DEFAULT NULL COMMENT 'ID додатку в офіційному каталозі',
  PRIMARY KEY (`id`),
  KEY `version` (`version`),
  KEY `name` (`name`),
